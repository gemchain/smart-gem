...
// Copyright (c) 2013 Smart Gem developers
...
const string strMessageMagic = "Smart Gem Signed Message:\n";
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("Smart GemMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("Smart GemMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running Smart GemMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("Smart GemMiner terminated\n");
        throw;
    }
}
